...
#///////////////////////////////////////////////////////////////////////////////////////
    ...
    # Descargar archivos de datos desde el bucket utilizando el cliente GCS
    for key, blob_name in files_to_download.items():
        try:
            client.download_blob_file(bucket_name, blob_name, local_file_paths[key])
        except Exception as e:
            print(f"Error al intentar descargar el archivo {blob_name}: {e}")
            # Si algo falla, se termina la ejecución
            return
...
# #///////////////////////////////////////////////////////////////////////    
    ...
    try:
        ...
        df_by_location_old = pd.read_csv(temp_local)
        df_by_location_old['date'] = pd.to_datetime(df_by_location_old['date'], format='%Y-%m-%d')
    ...
    df_by_location_new.to_csv(temp_local, index=False)
