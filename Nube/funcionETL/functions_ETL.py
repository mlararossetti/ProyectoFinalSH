...
def process_taxi_data(event, context):
    # Decodifica el mensaje del evento de Pub/Sub
    message_data = base64.b64decode(event['data']).decode('utf-8')
    print(f"Mensaje recibido: {message_data}")

    # Divide el mensaje en el texto principal y la fecha de inicio
    parts = message_data.split("|")
    if len(parts) == 2 and parts[0] == "Archivos cargados correctamente":
        bucket_name = "henry-taxi"  # Nombre del bucket
        start_date_str = parts[1]  # Extrae el start_date en formato string
        print(f"Procesando archivos para la fecha de inicio: {start_date_str}")

        # Convertir start_date_str a un objeto de fecha
        try:
            # Aquí se ajusta el formato de la fecha a "d/m/yyyy"
            start_date = datetime.strptime(start_date_str, "%Y-%m-%d")
            year_month = start_date.strftime("%Y-%m")  # Extrae el año y mes en formato YYYY-MM
            print(f"Fecha de procesamiento en formato año-mes: {year_month}")
        except ValueError as e:
            print(f"Error en el formato de start_date: {e}")
            return
