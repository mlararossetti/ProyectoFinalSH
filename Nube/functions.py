...
def get_start_date_from_csv(bucket_name, file_path):
    ...
    try:
        ...
        logging.error(f"Error leyendo gs://{bucket_name}/{file_path}: {e}")
        return datetime(2021, 1, 1)  # Valor predeterminado si no se encuentra el archivo
...
def download_and_upload_to_gcs(base_urls, output_folder, start_date, csv_url, fechas_csv, BUCKET_NAME, retries=3, delay=5):
    ...
    while current_date <= end_date:
        ...
        if all_files_downloaded:
            ...
            save_date_to_csv(current_date,BUCKET_NAME, fechas_csv)
